...
4. Test `release/IntegratedDataTool_Setup_vX.Y.Z.exe` in Windows Sandbox or a clean PC: install, first launch, OCR fallback, EML Chromium setup, and any required Office conversion.
5. Upload `release/`'s signed setup EXE, `App05_FileOps_vX.Y.Z.exe` launcher, and `.sha256` manifest to the GitHub Release. Publish the matching `vX.Y.Z` tag only after the artifact checks pass.
